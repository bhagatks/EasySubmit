...
| 2026-07-06 | **Enhance AI mission spec (aligned, not built)** — AI call kernel (shared classify/parse/retry/escalate + ledger) + failure UX (two buckets, surfaces, `resolveEnhanceOutcome`); Phases 0–4 in [`docs/enhance-ai-failure-ux.md`](./enhance-ai-failure-ux.md). |
...
| 2026-07-06 | Removed deprecated `app_config.aiConfig` — discovery defaults live in `dataRefresh`; system model/quotas in `aiEngine` |
| 2026-06-19 | `app_config` table + `prisma/seed.ts`: upserts `dataRefresh`, `ai_pricing_map`, and `enhanceWithAi` on deploy (`prisma db seed`) |
