...
- **Profile + Engine models** — `Profile` (many per `User`, one `isDefault`) with `Experience` / `Project` / `Education` / `Certification`; `Architecture` 1:1 `Profile` (`profileId`) for parsed JSON; multi-provider email linking via NextAuth
