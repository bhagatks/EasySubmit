...
- **Job Tracker (web v2)** — … **Review Screen** modal: Resume + Cover tabs share full-bleed preview, overlay toolbar (dark chrome), zoom controls; Cover inline edit + Save, **Enhance with AI** (cover brain + quota), pipeline seeds template cover on tailor; LaTeX editor; `reviewDocuments` on `job_resume_tailors`; …
