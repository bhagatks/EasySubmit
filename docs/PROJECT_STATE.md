...
- **Onboarding hub** — `/onboarding`: 3-phase Unified Workbench with unified top chrome (`OnboardingWorkbenchChrome`: EasySubmit brand + phase label/description + phase actions + Sign Out, progress bar, Identity \| Import \| Studio tabs); finalize CTA **Finalize & continue**; ATS sample PDF/DOCX in Import + Studio header actions; Studio validation runs **proactively after parse** (`ValidationErrorsBanner` + section highlights on Studio only, cleared when leaving Studio or re-uploading); Studio header: sample PDF/DOCX, raw text, expand/collapse, Import back
