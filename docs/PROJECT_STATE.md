...
- **Chrome extension v0.1** — MV3 single expandable job card (summary + inline job/resume/cover views), Apply journey, pipeline on all platforms, journey status above CTA, manual capture form; grip header order: resume profile → AI health alert → settings → close
