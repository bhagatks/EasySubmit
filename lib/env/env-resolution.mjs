...
/** Gitignored — prod DB URLs for local debug scripts only (never loaded by `next dev`). */
export const PROD_DEBUG_ENV_FILE = ".env.prod.local";
...
/** Database / Supabase keys allowed from `.env.prod.local` for prod debug scripts. */
export function loadProdDebugDatabaseVars(prodLocalVars) {
  const picked = {};
  for (const key of DATABASE_ENV_KEYS) {
    if (prodLocalVars[key] !== undefined && prodLocalVars[key] !== null) {
      picked[key] = String(prodLocalVars[key]).trim();
    }
  }
  return picked;
}
...
/**
 * Prod debug env — strips laptop DB URLs, merges `.env.prod.local` database keys only.
 * Refuses non-prod DATABASE_URL.
 */
export function resolveProdDebugEnvRecord(baseEnv, prodLocalVars = {}) {
  const picked = loadProdDebugDatabaseVars(prodLocalVars);
  if (!picked.DATABASE_URL) {
    return { env: null, error: "missing_database_url" };
  }
  if (!isProdDB(picked.DATABASE_URL)) {
    return { env: null, error: "not_prod_database_url" };
  }
  const env = mergeEnv(stripLocalDatabaseEnv(baseEnv), picked);
