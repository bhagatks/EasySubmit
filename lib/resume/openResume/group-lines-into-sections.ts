import type { ResumeKey } from "@/lib/resume/openResume/types";
import type {
  Line,
  Lines,
...
const isSectionTitle = (line: Line, lineNumber: number) => {
  ...
  if (isBold(textItem) && hasLetterAndIsAllUpperCase(textItem)) {
    ...
    const isKnownSection = SECTION_TITLE_KEYWORDS.some((keyword) =>
      lower.includes(keyword),
    );

    // Multi-word headers like "PROFESSIONAL EXPERIENCE" or "EDUCATION, CERTIFICATIONS & QUALIFICATIONS"
    if (isKnownSection && wordCount <= 10 && text.length <= 80) {
      return true;
    }

    // Short bold caps only (SKILLS, SUMMARY, etc.) — reject long headline-style lines
    if (wordCount <= 4 && text.length <= 42) {
      return true;
    }
