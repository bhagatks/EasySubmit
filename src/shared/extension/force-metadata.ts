import { isGenericNavigationJobTitle, scrapeDescription } from "./scrape-helpers";
...
function readMetaTitle(doc: Document): string {
  ...
  if (og && og.length > 2) {
    const cleaned = og.split("|")[0]?.trim() ?? og;
    if (!isGenericNavigationJobTitle(cleaned)) return cleaned;
  }

  const docTitle = doc.title?.split("|")[0]?.trim();
  if (docTitle && docTitle.length > 2 && !isGenericNavigationJobTitle(docTitle)) return docTitle;
...
function readHeadingTitle(doc: Document): string {
  for (const selector of [
    ...
    if (value && value.length > 2 && !isGenericNavigationJobTitle(value)) return value;
  }
  return "";
}
